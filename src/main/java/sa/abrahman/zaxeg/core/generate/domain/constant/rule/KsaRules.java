...
public class KsaRules {
    ...
    public static final String BR_KSA_10 = "[BR-KSA-10]: Buyer address must contain a street (BT-50), city (BT-52), country code (BT-55). This rule does not apply on the simplified tax invoices and associated credit notes and debit notes (KSA-2, position 1 and 2 = 02).";
    ...
    public static final String BR_KSA_08_14 = "[BR-KSA-08]: The seller identification (BT-29) must exist only once with one of the scheme ID (BT-29-1) (CRN, MOM, MLS, 700, SAG, OTH) and must contain only alphanumeric characters. In case of multiple commercial registrations, the seller should fill the commercial registration of the branch in respect of which the Tax Invoice is being issued. In case multiple IDs exist then one of the above must be entered following the sequence specified above.\n[BR-KSA-14]: The buyer identification (BT-46), required only if buyer is not VAT registered, then the buyer identification (BT-46) must be provided with one of the scheme IDs (BT-46-1) (TIN, CRN, MOM, MLS, 700, SAG, NAT, GCC, IQA, OTH) and must contain only alphanumeric characters. In case of multiple commercial registrations, the seller should fill the commercial registration of the branch in respect of which the Tax Invoice is being issued. In case multiple IDs exist then one of the above must be entered following the sequence specified above";
    ...
    public static final String BR_KSA_18 = "[BR-KSA-18]: VAT category code must contain one of the values (S, Z, E, O).";
    ...
    public static final String BR_KSA_40_44 = "[BR-KSA-40]: If it exists in the invoice, the seller VAT registration number or the seller group VAT registration number (BT-31) must contain 15 digits. The first and the last digits are “3”.\n[BR-KSA-44]: If it exists in the invoice, and If it is not an export invoice (KSA-2, position 5 is false), the buyer VAT registration number or buyer group VAT registration number (BT-48) must contain 15 digits. The first digit and the last digit is “3”.";
    ...
    public static final String BR_KSA_51 = "[BR-KSA-51]: The line amount with VAT (KSA12) must be Invoice line net amount (BT-131) + Line VAT amount (KSA-11).";
    public static final String BR_KSA_52 = "[BR-KSA-52]: The line VAT amount (KSA-11) is mandatory for tax invoice and associated credit notes and debit notes (KSA-2, position 1 and 2 = 01).";
    public static final String BR_KSA_53 = "[BR-KSA-53]: The line amount with VAT (KSA12) is mandatory for tax invoice and associated credit notes and debit notes (KSA-2, position 1 and 2 = 01).";
    ...
    public static final String BR_KSA_65 = "[BR-KSA-65]: Buyer Address Additional number (KSA-19) must be 4 digits if it exists";
    ...
    // KSA - CL
    public static final String BR_KSA_CL_02 = "[BR-KSA-CL-02]: All currencyID attributes (BT-5) must have the same value as the invoice currency code (BT-5), except for the invoice total VAT amount in accounting currency (BT111).";

...
@Component
@Order(ValidatorsOrderRegistry.METADATA_DATE_PRESENT)
public class ValidateInvoiceDatePresent implements InvoiceRuleValidator {
    private static final String PATH = "metadata.issueDate[present]";
    private static final String RULE = KsaRules.BR_KSA_04;

    @Override
    public FailableResult<Entry<String, String>> run(InvoiceGenerationPayload data) {
        if (data.getMetadata().getIssueDate().isAfter(LocalDate.now())) {

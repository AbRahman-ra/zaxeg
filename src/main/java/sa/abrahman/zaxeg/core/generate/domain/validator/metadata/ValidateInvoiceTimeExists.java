...
@Component
@Order(ValidatorsOrderRegistry.METADATA_TIME)
public class ValidateInvoiceTimeExists implements InvoiceRuleValidator {
    private static final String PATH = "metadata.issueTime";
    private static final String RULE = KsaRules.BR_KSA_70;

    @Override
    public FailableResult<Entry<String, String>> run(InvoiceGenerationPayload data) {
        if (data.getMetadata().getIssueTime() == null) {

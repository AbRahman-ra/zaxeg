...
@Component
@Order(ValidatorsOrderRegistry.PARTY_BUYER_ADDRESS_POSTAL_CODE)
public class ValidaterBuyerPostalCodeIfExists implements InvoiceRuleValidator {
    private static final String PATH = "parties.buyer.address.postalCode";
    private static final String RULE = KsaRules.BR_KSA_67;
    ...
    public @NonNull FailableResult<Entry<String, String>> run(InvoiceGenerationPayload data) {
        String postalcode = Optional.ofNullable(data.getParties().getBuyer()).map(PartiesPayload.Party::getAddress)
                .map(PartiesPayload.Address::getPostalCode).orElse(null);

        if (postalcode == null || postalcode.isBlank() || !postalcode.matches(MATCHER)) {

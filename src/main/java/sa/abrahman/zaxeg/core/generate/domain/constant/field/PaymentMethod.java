...
public enum PaymentMethod {
    /**
     * Not defined legally enforceable agreement between two or more parties (expressing a contractual right or a right
    ...
    /**
     * Cheque drawn by a bank on itself or its agent. A person who owes money to another buys the draft from a bank for
     * cash and hands it to the creditor who need have no fear that it might be dishonoured.
     */
    CERTIFIED_BANKER_DRAFT(22),

    /**
     * Payment by a pre-printed form, which has been completed by a financial institution, on which instructions are
     * given to an account holder (a bank or building society) to pay a stated sum to a named recipient.
     */
    BANK_CHEQUE(23),
    ...
    /**
     * Payment by a pre-printed form stamped with the paying bank's certification on which instructions are given to an
     * account holder (a bank or building society) to pay a stated sum to a named recipient .
     */
    CERTIFIED_CHEQUE(25),
    ...
    /**
     * A credit transaction made through the ACH system to asavings account using the CCD+ payment format.
     */
    ACH_SAVINGS_CCD_PLUS_CREDIT(41),

    /**
     * Payment by an arrangement for settling debts that isoperated by the Post Office.
     */
    PAYMENT_TO_BANK_ACCOUNT(42),

    /**
     * A debit transaction made through the ACH system to asavings account using the CCD+ payment format.
     */
    ...
    /**
     * The amount is to be, or has been, directly debited tothe customer's bank account.
     */
    DIRECT_DEBIT(49),

    /**
     * A method for the transmission of funds through thepostal system rather than through the banking system.
     */
    PAYMENT_BY_POSTGIRO(50),

    /**
     * A French standard procedure that allows a debtor to payan amount due to a creditor. The creditor will forwardit
     * to its bank, which will collect the money on the bankaccount of the debtor.
     */
    FR_NORME_6_97_TELEREGLEMENT_CFONB_OPTION_A(51),

    /**
     * Payment order which requires guaranteed processing bythe most appropriate means to ensure it occurs on
     * therequested execution date, provided that it is issued tothe ordered bank before the agreed cut-off time.
     */
    URGENT_COMMERCIAL_PAYMENT(52),

    /**
     * Payment order or transfer which must be executed, by themost appropriate means, as urgently as possible andbefore
     * urgent commercial payments.
     */
    ...
    /**
     * The payment means have been previously agreed betweenseller and buyer and thus are not stated again.
     */

...
public abstract class AbstractUBLInvoiceValidator implements InvoiceValidator {
    ...
    @Override
    public void validate(InvoiceGenerationPayload payload) throws InvoiceRuleViolationException {
        validateBusinessRules(payload);
        validateSpecificRules(payload);
    }

    protected void validateBusinessRules(InvoiceGenerationPayload invoice) {
        // Function<String, RuntimeException> f = InvoiceRuleViolationException::new;

        // if (invoice.getDocumentAllowanceCharges() != null) {
        // String rule31Or36 = "BR-31/BR-36: Each Document level allowance/charge shall
        // have a Document level allowance/charge amount";
        // String rule32Or37 = "BR-32/BR-37: Each Document level allowance/charge shall
        // have a Document level allowance/charge VAT category code";
        ...
        // String rule53 = "BR-53: If the VAT accounting currency code is present, then
        // the Invoice total VAT amount in accounting currency shall be provided";
        // ObjectValueValidator.check(invoice.getTaxCurrency(), f)
        // .matches((c) ->
        // c.equals(invoice.getFinancials().getTotalTaxAmountInAccountingCurrency()),
        // rule53);
        // }

        // if (invoice.getBillingReference() != null) {
        // String rule55 = "BR-55: Each Preceding Invoice reference shall contain a
        // Preceding Invoice reference";
        // StringValueValidator.check(invoice.getBillingReference().getOriginalInvoiceNumber(),
        // f).exists(rule55);
        // }
    ...
    protected void validateAdditionalBusinessRules(InvoiceGenerationPayload invoice) {
    };

    protected abstract void validateSpecificRules(InvoiceGenerationPayload invoice);
}

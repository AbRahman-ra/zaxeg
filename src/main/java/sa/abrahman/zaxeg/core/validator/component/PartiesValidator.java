...
public class PartiesValidator implements InvoiceValidator {
    ...
    public void validate(InvoiceGenerationPayload payload) {
        ...
        Predicate<PartiesPayload.Address> isCompliantSaudiPartyAddress = a -> !a.getStreet().isBlank()
                && !a.getBuildingNumber().isBlank()
                && !a.getPostalCode().isBlank()
                && !a.getCity().isBlank()
                && !a.getDistrict().isBlank()
                && a.getCountry() != null;
        ...
        ObjectValueValidator.check(sellerAddress, f).exists(UblRules.BR_08).matches(isCompliantSaudiPartyAddress, KsaRules.BR_KSA_09);
        StringValueValidator.check(sellerAddress.getBuildingNumber(), f).hasLength(4, KsaRules.BR_KSA_37).numeric(KsaRules.BR_KSA_37);

        if (!sellerAddress.getAdditionalNumber().isBlank()) {
            StringValueValidator.check(sellerAddress.getAdditionalNumber(), f).hasLength(4, KsaRules.BR_KSA_64).numeric(KsaRules.BR_KSA_64);
        }

        StringValueValidator.check(sellerAddress.getPostalCode(), f).hasLength(5, KsaRules.BR_KSA_66).numeric(KsaRules.BR_KSA_66);
        ...
        .hasLength(15, KsaRules.BR_KSA_40)
        .startsAndEndsWith("3", KsaRules.BR_KSA_40);
        StringValueValidator.check(seller.getName(), f).exists(UblRules.BR_06);

        if (buyer != null) {
            PartiesPayload.Address buyerAddress = buyer.getAddress();
            ObjectValueValidator.check(buyerAddress, f).exists(UblRules.BR_10).matches(isCompliantBuyerAddress, KsaRules.BR_KSA_10);
            if ("sa".equalsIgnoreCase(buyerAddress.getCountry().getCountry())) {
                ObjectValueValidator.check(buyerAddress, f).exists(UblRules.BR_10).matches(isCompliantSaudiPartyAddress, KsaRules.BR_KSA_63);
                StringValueValidator.check(buyerAddress.getPostalCode(), f).hasLength(5, KsaRules.BR_KSA_67).numeric(KsaRules.BR_KSA_67);
            }

package sa.abrahman.zaxeg.core.validator.aggregate;
...
public class PayloadAggregatesValidator implements InvoiceValidator {
    ...
    private void metadataAndPartiesValidations(InvoiceGenerationPayload payload) {
        ...
        if (isExportInvoice) {
            ...
            ObjectValueValidator.check(buyerVatInfo, f).matches(i -> i == null || i.getCompanyId() == null || i.getCompanyId().isBlank(), KsaRules.BR_KSA_46);
        }
    ...
    private void partiesAndCheckoutDetailsValidations(InvoiceGenerationPayload payload) {
        ...
        if (payload.getCheckout() != null) {
            boolean hasEducationalOrHealthExemption = payload.getCheckout().getInvoiceTaxTotals().getTaxSubtotal().stream()
                    .map(TaxSubtotal::getTaxCategory)
                    .filter(Objects::nonNull)
                    .anyMatch(tax ->
                        tax.getTaxExemptionReasonCode() == TaxExemptionCode.PRIVATE_EDUCATION_TO_CITIZEN ||
            ...
            if (hasEducationalOrHealthExemption) {
                ...
                CollectionValueValidator.check(buyerIds, f)
                        .exists(KsaRules.BR_KSA_49)
                        .notEmpty(KsaRules.BR_KSA_49)

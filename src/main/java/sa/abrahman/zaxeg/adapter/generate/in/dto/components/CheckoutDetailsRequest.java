...
public class CheckoutDetailsRequest implements Mapable<CheckoutDetailsPayload, List<Currency>> {

    @NotNull(message = UblRules.BR_49)
    ...
    @Override
    public CheckoutDetailsPayload mapped(List<Currency> additionalData) {
        ...
        return CheckoutDetailsPayload.builder().paymentMeansType(paymentMeansType).paymentTerms(paymentTerms)
                ...
                .documentLevelAllowanceCharges(
                        documentLevelAllowanceCharges.stream().map(ac -> ac.mapped(documentCurrency)).toList())
                .legalMonetaryTotals(legalMonetaryTotals.mapped(documentCurrency))
                .invoiceTaxTotals(invoiceTaxTotals.mapped(documentCurrency))
                .invoiceTaxTotalsInAccountingCurrency(invoiceTaxTotalsInAccountingCurrency.mapped(taxCurrency)).build();
    }

    @Getter
    @Schema(description = "Client-provided totals for strict mathematical verification")
    static class LegalMonetaryTotals implements Mapable<CheckoutDetailsPayload.LegalMonetaryTotals, Currency> {
        ...
        @Override
        public CheckoutDetailsPayload.LegalMonetaryTotals mapped(Currency currency) {

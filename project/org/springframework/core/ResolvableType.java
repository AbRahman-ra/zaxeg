...
implements Serializable {
    ...
    boolean isEntirelyUnresolvable() {
        ...
        for (ResolvableType generic2 : generics = this.getGenerics()) {
            if (generic2.isUnresolvableTypeVariable() || generic2.isWildcardWithoutBounds()) continue;
            return false;
        }
        return true;
    }
    ...
    public ResolvableType getGeneric(@Nullable int ... indexes) {
        ...
        ResolvableType generic2 = this;
        for (int index : indexes) {
            generics = generic2.getGenerics();
            if (index < 0 || index >= generics.length) {
                return NONE;
            }
            generic2 = generics[index];
        }
        return generic2;
    }
    ...
    public static ResolvableType forClassWithGenerics(Class<?> clazz, ResolvableType ... generics) {
        ...
        for (int i = 0; i < generics.length; ++i) {
            ResolvableType generic2 = generics[i];
            Type argument = generic2 != null ? generic2.getType() : null;
